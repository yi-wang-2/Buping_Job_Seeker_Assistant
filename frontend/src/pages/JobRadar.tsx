...
export default function JobRadarPage({ t }: { t: Strings }) {
  ...
  const renderJobCard = (job: JobRecommendation, featured = false) => (
    <article key={job.id} className={`rounded-2xl border bg-white p-5 shadow-sm transition hover:-translate-y-0.5 hover:shadow-md dark:bg-gray-800 ${featured ? "border-amber-300 dark:border-amber-700" : "border-gray-200 dark:border-gray-700"}`}>
      ...
      <div className="mt-5 flex flex-wrap items-center justify-between gap-3 border-t border-gray-100 pt-4 dark:border-gray-700"><div className="text-[11px] text-gray-400">{english ? "Seen" : "发现于"} {new Date(job.first_seen_at).toLocaleDateString()}</div><div className="flex flex-wrap gap-2"><button onClick={() => updateAction(job, "favorite")} className={`inline-flex items-center gap-1 rounded-lg border px-3 py-2 text-xs ${job.favorite ? "border-rose-300 bg-rose-50 text-rose-600" : "border-gray-200 text-gray-600 dark:border-gray-600 dark:text-gray-300"}`}><Heart className={`h-3.5 w-3.5 ${job.favorite ? "fill-current" : ""}`} />{job.favorite ? "已收藏" : "收藏"}</button><button onClick={() => updateAction(job, "not_interested")} className={`inline-flex items-center gap-1 rounded-lg border px-3 py-2 text-xs ${job.not_interested ? "border-gray-400 bg-gray-100 text-gray-700" : "border-gray-200 text-gray-600 dark:border-gray-600 dark:text-gray-300"}`}><ThumbsDown className="h-3.5 w-3.5" />{job.not_interested ? "已标记" : "不感兴趣"}</button>{job.link && <a href={job.link} target="_blank" rel="noreferrer" className="inline-flex items-center gap-1 rounded-lg border border-gray-200 px-3 py-2 text-xs text-gray-600 hover:bg-gray-50 dark:border-gray-600 dark:text-gray-300"><ExternalLink className="h-3.5 w-3.5" />查看</a>}<button onClick={() => track(job)} disabled={tracked[job.id] || job.applied} className="inline-flex items-center gap-1 rounded-lg bg-brand-600 px-3 py-2 text-xs font-semibold text-white hover:bg-brand-700 disabled:bg-emerald-600">{tracked[job.id] || job.applied ? <Check className="h-3.5 w-3.5" /> : <WandSparkles className="h-3.5 w-3.5" />}{tracked[job.id] || job.applied ? "已投递/记录" : "加入求职记录"}</button></div></div>
    </article>
  );
